...
def get_news(search_text : str) -> pd.DataFrame:
    ...
    top = gn.search(search_text, when = '1d')
    entries = top["entries"]
...
def build_similarity_matrix(sentences : list[str], nlp) -> np.matrix:
    ...
    for idx1 in range(len(sentences)):
        for idx2 in range(len(sentences)):
            if idx1!=idx2:
                similarity_matrix[idx1][idx2] = sentence_similarity(sentences[idx1].text,sentences[idx2].text, nlp)
    return similarity_matrix

def tl_dr(title, title_detail, links, link, id, guidislink, published, published_parsed, summary, summary_detail, source, sub_articles):
...
if __name__ == "__main__":
    ...
    if df.shape[0]:
        with WorkerPool(n_jobs=os.cpu_count() - 1) as pool:
            results = pool.map(tl_dr, list(df.itertuples(index=False, name=None)))
        stats_news = []
        for result in results:
            try:
                st.markdown('### <a href="{}" rel="noopener noreferrer" target="_blank">{}</a>'.format(result['link'], result['title']),unsafe_allow_html=True)
                col1, col2 = st.columns(2)
                col1.markdown(result['link'].replace("https://","").replace("www.","").split("/")[0])
                col2.markdown(result['published_date'] if result['published_date'] != "None" else "")
                try:
